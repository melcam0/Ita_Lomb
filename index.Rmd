...
X<-covid
Giorno<-c(1:nrow(X))
X<-cbind.data.frame(Giorno,X$ricoverati_con_sintomi,X$terapia_intensiva,X$isolamento_domiciliare,X$dimessi_guariti,X$deceduti)
Y<-X
for(i in 2:nrow(X))Y[i,2:6]<-X[i,2:6]-X[i-1,2:6]
colnames(Y)<-c('giorno','ricoverati','t_intensiva','i_domiciliare','guariti','morti')

n<-nrow(Y)

Y<-cbind.data.frame(mese=months(unique(covid$data)),Y)
Y$mese<-factor(Y$mese,levels=c('febbraio','marzo','aprile','maggio','giugno','luglio','agosto','settembre','ottobre','novembre'))
...
X<-cbind.data.frame(Giorno,X$ricoverati_con_sintomi,X$terapia_intensiva,X$isolamento_domiciliare,X$dimessi_guariti,X$deceduti)
Z<-X
for(i in 2:nrow(X))Z[i,2:6]<-X[i,2:6]-X[i-1,2:6]
colnames(Z)<-c('giorno','ricoverati','t_intensiva','i_domiciliare','guariti','morti')

n<-nrow(Z) 

Z<-cbind.data.frame(mese=months(unique(covid$data)),Z)
Z$mese<-factor(Z$mese,levels=c('febbraio','marzo','aprile','maggio','giugno','luglio','agosto','settembre','ottobre','novembre'))
...
=======================================================================
```{r}
DT::datatable(data = Y[1:(m-1),],rownames = FALSE)
```
...
=======================================================================
```{r}
DT::datatable(data = Y[m:p,],rownames = FALSE)
```
...
### {data-height=350}
Consideriamo le 5 variabile dell'andamento giornaliero italiano: ricoverati con sintomi, terapia intensiva, isolamento domiciliare, guariti e morti. 
...
La linea blu è Italia e la verde Lombardia.
