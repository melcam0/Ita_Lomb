...
```{r}
Y[,8]<-round(Y[,8]*100,2)
DT::datatable(data = Y[1:(m-1),],rownames = FALSE, extensions = 'Buttons',
...
```{r}
DT::datatable(data = Y[m:p,],rownames = FALSE, extensions = 'Buttons',
              options = list(order = list(list(1, 'desc')),dom = 'Blfrtip',
                             buttons = list(list(extend = 'collection',buttons = c('csv', 'excel', 'pdf'),text = 'Download')) 
                                                                  ))
```
...
Consideriamo le 6 variabile dell'andamento giornaliero italiano (quantità): ricoverati con sintomi, terapia intensiva, isolamento domiciliare, guariti morti e tasso di positività (positivi/tamponi).
...
La prima componente "legge" la gravità della situazione in generale (in particolare sul sistema sanitario).
...
La seconda componente "legge" il tasso di positività vs dimessi/guariti e isolamento domiciliare. La guarigione è sia clinica (dimessi) che virologica (positivi diventati negativi) il che rende la variabile un po' ingannevole

L'andamento nel tempo della seconda componente (o meglio dellla media mobile a 7 giorni - per "smorzare" le oscillazioni giornaliere ) "legge" soprattutto il tasso di positività.
